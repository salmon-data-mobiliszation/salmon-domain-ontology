...
- The live W3ID publication surface now uses DFO-style root + SemVer content negotiation for the shared ontology root and release paths, while representative module/build/profile secondary surfaces remain Turtle-first.
- Canonical shared-term IRIs dereference to WIDOCO term anchors for humans by default while keeping machine-readable serializations negotiable.
...
What changed after the initial registration is the live public routing contract itself: W3ID now serves the richer content-negotiated/versioned surface for the shared ontology root and SemVer paths, aligned to the same root + SemVer pattern used by the live DFO Salmon Ontology W3ID rules. The local mirror for that live state remains `docs/publishing/w3id-smn-draft/.htaccess`.

The term-path routing follow-up is now live:

- `https://w3id.org/smn/<Term>` resolves by default to `https://salmon-data-mobilization.github.io/salmon-domain-ontology/#/<Term>`
- the same term paths return `303` redirects to `smn.ttl`, `smn.owl`, or `smn.jsonld` when requested via `Accept`
- research / case-study / module / profile secondary surfaces remain Turtle-first

That contract is mirrored in `docs/publishing/w3id-term-dereference-draft/` and was merged upstream in `perma-id/w3id.org` PR #5881. See also `w3id-request-payload.md`, `docs/publishing/evidence/2026-03-13-w3id-live-redirect-check.md` (historical initial state), `docs/publishing/evidence/2026-03-26-w3id-content-negotiation-live-check.md` (root + SemVer live state), and `docs/publishing/evidence/2026-05-13-w3id-term-dereference-live-check.md` (current term-path live state).
